			...
			if (reps[name]) then
				...
				if (difference > 0) then
					--ChatFrame5:AddMessage(format("Reputation with %s increased by %d. (%d remaining until %s)", --long version
                    				ChatFrame5:AddMessage(format("%s + %d. (%d until %s)",
					name, difference, topValue - earnedValue, (standingID == 8) and "max" or _G["FACTION_STANDING_LABEL"..standingID + 1]), 0.5, 0.5, 1)
				elseif (difference < 0) then
					difference = 0 - difference
					--ChatFrame5:AddMessage(format("Reputation with %s decreased by %d. (%d remaining until %s)", --long version
                    				ChatFrame5:AddMessage(format("%s - %d. (%d until %s)",
					name, difference,  earnedValue - bottomValue, (standingID == 1) and "min" or _G["FACTION_STANDING_LABEL"..standingID - 1]), 0.5, 0.5, 1)
				end
			end
